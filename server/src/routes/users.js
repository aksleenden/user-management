...
const SORTABLE_FIELDS = ['name', 'email', 'status', 'lastLoginTime', 'registrationTime'];

router.get('/', async (req, res, next) => {
  try {
    const { search = '', sortBy = 'lastLoginTime', order = 'desc' } = req.query;

    const sortField = SORTABLE_FIELDS.includes(sortBy) ? sortBy : 'lastLoginTime';
    ...
    const users = await User.find(filter).sort({ [sortField]: sortOrder, _id: 1 }).lean();
...
router.post('/delete-unverified', async (req, res, next) => {
  try {
    ...
    const result = await User.deleteMany({ _id: { $in: ids }, status: 'unverified' });

    if (result.deletedCount === 0) {
      return res.json({ message: 'None of the selected users were unverified', deletedCount: 0 });
    }
